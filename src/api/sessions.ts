  ...
  async create(sessionData: Omit<CreateSessionDto, 'session_number'>) {
    ...
    try {
      ...
      try {
        ...
        const immediateHeading = immediateClientName ? `Создана сессия с ${immediateClientName}` : 'Создана сессия';
        ...
        if (reminderAt.getTime() > Date.now()) {
          ...
          const source = (createdSession.clients && (createdSession as any).clients?.source) ? (createdSession as any).clients.source : undefined;
